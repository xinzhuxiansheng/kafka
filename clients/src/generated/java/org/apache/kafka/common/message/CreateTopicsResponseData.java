    ...
    static public class CreatableTopicResult implements Message, ImplicitLinkedHashMultiCollection.Element {
        ...
        public void write(Writable _writable, ObjectSerializationCache _cache, short _version) {
            ...
            if (_version >= 5) {
                ...
                if (topicConfigErrorCode != (short) 0) {
                    throw new UnsupportedVersionException("Attempted to write a non-default topicConfigErrorCode at version " + _version);
                }
            }
            if (_version >= 5) {
                ...
                if (numPartitions != -1) {
                    throw new UnsupportedVersionException("Attempted to write a non-default numPartitions at version " + _version);
                }
            }
            if (_version >= 5) {
                ...
                if (replicationFactor != (short) -1) {
                    throw new UnsupportedVersionException("Attempted to write a non-default replicationFactor at version " + _version);
                }
            }
            if (_version >= 5) {
                if (configs == null) {
                ...
                if (!configs.isEmpty()) {
                    throw new UnsupportedVersionException("Attempted to write a non-default configs at version " + _version);
                }
            }
            RawTaggedFieldWriter _rawWriter = RawTaggedFieldWriter.forFields(_unknownTaggedFields);
            _numTaggedFields += _rawWriter.numFields();
